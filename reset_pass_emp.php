...
if (isset($_POST['reset'])) {
    ...
    $cpass =$_POST["cpass"];
      ...
      $sql = "SELECT password FROM employee WHERE empid = '$id'";
      $statement = $conn->prepare($sql);  
      $statement->execute();  
      $emp_data = $statement->fetchAll();
//        print_r($emp_data);
      if(($emp_data[0]["password"]=="$oldpass") and ("$newpass"=="$cpass")){
        $sql="update employee set password='$cpass' where empid='$id'";
        $statement = $conn->prepare($sql);  
